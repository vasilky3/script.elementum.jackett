...
def tv_season_episode(results, season, season_name, episode, global_ep, ep_year, season_year=0, start_year=0):
    ...
    for res in results:
        ...
        year_pattern = "(?P<from>(?:19|20)\d{2})(?:\s*-\s*(?P<to>[12]\d{3}))?"
        ...
        season_pattern = "\W(?P<s_flag>s|season|сезон)[\s\(\[\{]*(?P<from>\d+)(?:\s*-\s*(?P<to>\d+))?"
        s = re.search(season_pattern, name_no_year)
        if s:
            s_from = int(s.group("from") or "99999")
            s_to = int(s.group("to") or "-1")
            s_flag = s.group("s_flag")
            if season == s_from or (s_from <= season <= s_to):  # season is sutable
                filtered.append(res)
                continue
        ...
        if not global_ep:
            continue
        episode_pattern = "(?:e?(?P<from>\d+)(?:\s*-\s*e?(?P<to>\d+)))|(?P<last>\d+)(?:\s*\+\s*\d*)?(?:\s*(из|of)\s*(?P<all>\d+))"
